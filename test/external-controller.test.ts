...
describe('external worker controller', () => {
  it('creates one stable worker for an exact idempotent ensure replay', () => {
    ...
    const first = ensureExternalWorker({
      controllerId: 'codex',
      workerKey: 'frontend',
      operationId: 'ensure-1',
      task: 'Implement the frontend change.'
    });
  ...
  it('isolates identical worker keys belonging to different controllers', () => {
    const a = ensureExternalWorker({
      controllerId: 'codex', workerKey: 'worker', operationId: 'a-1', task: 'A'
    });
    const b = ensureExternalWorker({
      controllerId: 'omp', workerKey: 'worker', operationId: 'b-1', task: 'B'
    });
  ...
  it('sends an idempotent follow-up to the same persistent worker', () => {
    const worker = ensureExternalWorker({
      controllerId: 'codex', workerKey: 'frontend', operationId: 'ensure-1', task: 'A'
    });
  ...
  it('preserves worker and operation identity across restart restore', () => {
    ...
    bindExternalWorkerConversation(worker.runId, 'chat-provider-123');
    sendExternalWorkerMessage({
      controllerId: 'codex', workerKey: 'frontend', operationId: 'msg-1', text: 'Continue.'
    });
